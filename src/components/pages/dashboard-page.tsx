...
import { chartAxisTick, chartGridStroke, chartMutedTick, chartTooltipStyle } from '@/lib/charts'
import { complianceTrend, incidentTrend, kloeScores } from '@/lib/mock-data'
import { complianceScore, openActionCount, openIncidentCount } from '@/lib/metrics'

export function DashboardPage() {
  const data = useHavenData()
  const score = complianceScore(data)
  const openActions = openActionCount(data)
  const openIncidents = openIncidentCount(data)

  return (
    <>
      <PageHeader
        title={`Good morning, ${data.session?.name?.split(' ')[0] || 'there'}`}
        description={`Here’s what’s happening across ${data.session?.campusName || 'the home'} today.`}
        action={
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { href: '/medication', label: 'Medication', detail: `${data.medications.length} MAR entries`, icon: Pill },
          { href: '/training', label: 'Training', detail: `${data.trainings.length} courses`, icon: GraduationCap },
          { href: '/enquiries', label: 'Enquiries', detail: `${data.enquiries.length} in pipeline`, icon: Inbox },
          { href: '/cqc', label: 'CQC board', detail: 'Five key questions', icon: ClipboardCheck }
        ].map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-blue-200 hover:bg-blue-50/40"
            >
          ...
          <CardContent className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={complianceTrend} margin={{ left: -18, right: 8, top: 12 }}>
                ...
                <YAxis domain={[70, 100]} axisLine={false} tickLine={false} tick={chartMutedTick} />
          ...
          <CardContent className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={incidentTrend} margin={{ left: -25, right: 6, top: 12 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartGridStroke} />
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={chartAxisTick} />
                <YAxis axisLine={false} tickLine={false} tick={chartMutedTick} />
