...
| `src/components/haven-app.tsx` | Application shell: sidebar, header, page routing |
| `src/components/navigation.ts` | Navigation registry and the menu-access rule used by the shell |
...
| `src/lib/server/route.ts` | `withMenu` / `withSession` handler guards and JSON body parsing |
| `src/lib/server/uploads.ts` | Resolves `UPLOAD_DIR` and bounds stored document paths |
