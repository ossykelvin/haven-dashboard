...
import { complianceScore, incidentCounts, openActionCount, openIncidentCount, staffCompliancePercent, unreadNotificationCount } from '../src/lib/metrics'
...
describe('Haven dashboard metrics', () => {
  ...
  it('tracks incident and notification states', () => {
    ...
    expect(
      openIncidentCount({
        incidents: [{ status: 'Open' }, { status: 'Investigating' }, { status: 'Closed' }]
      } as never)
    ).toBe(2)

    expect(
      staffCompliancePercent({
        staff: [{ status: 'Compliant' }, { status: 'Due soon' }, { status: 'Compliant' }]
      } as never)
    ).toBe(67)
  })
})
