...
### 2026-09-20 — HAVEN-003 — MySQL backend and authenticated modules

**Status:** DONE.

Replaced localStorage with MySQL tables matching the CompliCare `tbl_*` catalogue (`tbl_auth_user` stands in for Supabase Auth). Session JWT, campus/menu authorization, MAR rounds, messages, users/roles/campuses, document files, CSV import, email outbox, and `tbl_change_audit` are live. Register dates now map from care-plan entries; create forms keep a form reference across await so they can reset after save.

**Validation:** `npm run verify` passed (ESLint, 24 Vitest tests, TypeScript, Next.js production build). Browser smoke: demo admin login, dashboard, residents create (Helen Foster), MAR round, messages UI, emails page, documents upload control, data import, change audit (resident + MAR rows), staff, sign-out to `/login`. File-picker import/upload was not exercised in the browser.

**Blockers / gaps:** SMTP/MFA, production identity, and live care-record accreditation remain out of scope. MySQL has no RLS; the API is the security boundary.

### 2026-09-20 — HAVEN-002 — CompliCare module expansion

**Status:** DONE.

Added demonstration modules aligned to CompliCare: Rota, Medication, Training, Maintenance, Enquiries, CQC KLOE board, Documents, and Risks. Sidebar is grouped into Overview, Operations, Enquiries, and Compliance. Create dialogs, Zod schemas, record factories, and localStorage merge cover the new collections. Authentication, tenancy, email, file upload, and server persistence were not copied.

**Validation:** `npm run verify` passed (ESLint, 17 Vitest tests, strict TypeScript, Next.js production build). Browser smoke confirmed Dashboard shortcuts plus `/medication`, `/rota`, `/training`, `/maintenance`, `/enquiries`, `/cqc`, `/risks`, and `/documents`.

**Blockers / gaps:** Full MAR rounds, enquiry sub-pages, chat, users/roles, and real document files remain out of scope for the client prototype.

### 2026-09-20 — HAVEN-001 — Dashboard code refactor

**Status:** DONE.

Collapsed duplicated register-page, create-dialog, persistence, and domain-option code into shared helpers without changing product behaviour. Shared option lists now feed schemas, forms, and filters; record factories and storage helpers sit outside the React provider; register pages use common StatGrid / FilterBar / RegisterCard primitives; navigation metadata and page components share one route map.

**Validation:** `npm run verify` passed (ESLint, 14 Vitest tests, strict TypeScript, Next.js production build). Browser smoke at localhost:3000 confirmed all nine routes, resident search, add-resident dialog, incident Open tab, unread notification filter, reports charts/export control, and register stats. Create-form date fields were not submitted in the browser (dialog open/cancel only).

**Blockers / gaps:** Next.js still surfaces a hydration overlay around locale date formatting in the client shell; it predates this split and does not change register behaviour.
...
**Validation:** `npm run verify` passed locally, loading `next.config.mjs` without the TypeScript-config SWC path. GitHub Actions run `35648348066` passed and appended a normal commit to `cpanel-release`.
...
**Validation:** GitHub Actions run `35646440689` passed verification and republished `cpanel-release` with the clean-tree compatibility changes.
...
**Validation:** GitHub Actions run `35644829193` passed verification and republished `cpanel-release` with the `.htaccess` safeguard.
